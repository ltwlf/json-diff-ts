...
const compare = (
    ...
    switch (typeOfOldObj) {
        ...
        case 'Array':
            changes = changes.concat(
                compareArray(oldObj, newObj, path, embeddedObjKeys, keyPath)
            )
            break
        case 'Function':
            break
...
const compareArray = (
    ...
    const left =
        embeddedObjKeys != null ? embeddedObjKeys[keyPath.join('.')] : undefined
        ...
        for (const subchange of change.changes) {
            if (subchange.value != null || subchange.type === Operation.REMOVE) {
                ...
                if (change.embeddedKey === '$index') {
                    ...
                    element = find(
                        arr,
                        el => el[change.embeddedKey].toString() === subchange.key.toString()
                    )
                }
        ...
        for (const subchange of change.changes) {
            if (subchange.value != null || subchange.type === Operation.REMOVE) {
                ...
                if (change.embeddedKey === '$index') {
                    ...
                    element = find(
                        arr,
                        el => el[change.embeddedKey].toString() === subchange.key
                    )
                }
...
export const diff = (
    oldObj: any,
    newObj: any,
    embeddedObjKeys?: Dictionary<string>
): IChange[] => compare(oldObj, newObj, [], embeddedObjKeys, [])
...
export const flattenChangeset = (
    ...
    if (Array.isArray(obj)) {
        return obj.reduce(
            (memo, change) => [
                ...memo,
                ...flattenChangeset(change, path, embeddedKey),
            ],
            [] as IFlatChange[]
        )
    } else {
